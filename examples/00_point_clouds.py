...
def main(args):
    ...
    bpy.ops.object.light_add(type="SUN", radius=1, align="WORLD", location=[1, 0, 2], rotation=[1, 0, 2], scale=[1, 1, 1])
    bpy.context.object.data.energy = 1
    bpy.context.scene.world.color = (1, 0.934681, 0.78918)
    ...
    out_path = osp.join(root, filename + '.blender')
    ...
    # poincloud_material = PrincipledBSDFMaterial(specular=specularity, roughness=roughness)
    # pointcloud_colors_init = VertexColors(np.asarray(data.rgb_2).astype('float32') / 255.)
    # pointcloud = scene.renderables.add_pointcloud(
    #     vertices=np.asarray(data.pos), material=poincloud_material, colors=pointcloud_colors_init, point_size=0.03,
